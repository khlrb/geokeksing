...
def add_kekse():
	if request.form['title'] and request.form['creator'] and request.form['latitude'] and request.form['longitude'] and request.form['description']:
		...
		g.db.execute('insert into kekse (title, creator, description, latitude, longitude, secret) values (?, ?, ?, ?, ?, ?)', [request.form['title'], request.form['creator'], request.form['description'], request.form['latitude'], request.form['longitude'], hash])
		g.db.commit()
...
def remove_kekse():
	if request.form['secret']:
		g.db.execute('delete from kekse where secret = ?', [request.form['secret']])
		g.db.commit()
	return redirect(url_for('show_map'))
